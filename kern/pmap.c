...
page_init(void)
{
	...
	// free pages!
	size_t i;
	uint32_t n_left = IOPHYSMEM/PGSIZE;
	uint32_t n_right = ((uint32_t) boot_alloc(0) - KERNBASE)/PGSIZE;
	page_free_list = NULL;
	for (i = 0; i < npages; i++) {
		if(i == 0)
			pages[i].pp_ref = 1;
		else if(i >= n_left && i < n_right)
			pages[i].pp_ref = 1;
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref > 0)
		panic("page_free: Current pp_ref is nonzero!\n");
	if(pp->pp_link)
		panic("page_free: Current pp_link is not NULL!\n");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(pde & PTE_P))
	{
		if(create)
		{
			pagenew = page_alloc(ALLOC_ZERO);
			if(!pagenew)
				return NULL;
			++(pagenew->pp_ref);
			pgdir[PDX(va)] = page2pa(pagenew)|PTE_P|PTE_U|PTE_W;//why pte flags?
			return (pte_t *)page2kva(pagenew) + PTX(va);
		}
		else
			return NULL;
	}
