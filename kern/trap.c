...
trap_init(void)
{
	...
	{
		SETGATE(idt[cnt], 0, GD_KT, trap_handlers[cnt], 0);
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(curenv->env_pgfault_upcall)
	{
		...
		if(tf->tf_esp<UXSTACKTOP && tf->tf_esp>UXSTACKTOP-PGSIZE)
			current_esp = tf->tf_esp - sizeof(uint32_t) - sizeof(struct UTrapframe);
		else
			current_esp = UXSTACKTOP - sizeof(struct UTrapframe);

		user_mem_assert(curenv, (void *)current_esp, sizeof(struct UTrapframe), PTE_P|PTE_U|PTE_W);

		utf = (struct UTrapframe *)current_esp;
